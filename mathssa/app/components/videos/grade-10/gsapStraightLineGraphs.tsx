...
export default function StraightLineGraphsGSAP() {
  ...
  // the "stages" GSAP will tween — same numbers your original derived from `p`
  const stage = useRef({
  ...
  function drawAxes(ctx: CanvasRenderingContext2D) {
    ...
    ctx.fillStyle = '#8BA7C4'; ctx.font = '14px Arial'
    ctx.textAlign = 'center'; ctx.fillText('x', W - 20, OY + 4)
    ctx.textAlign = 'left'; ctx.fillText('y', OX + 6, 42)
  }
  ...
  function drawScene(ctx: CanvasRenderingContext2D) {
    ...
    if (s.labelOpacity > 0) {
      ...
      ctx.fillStyle = '#0D1B2A'; ctx.font = 'bold 22px Arial'; ctx.textAlign = 'left'
    ...
    if (s.angleOpacity > 0) {
      ctx.globalAlpha = s.angleOpacity
      ctx.fillStyle = '#556B82'; ctx.font = '14px Arial'; ctx.textAlign = 'left'
      ctx.fillText('rise/run shown here', W - 220, 74)
      ctx.globalAlpha = 1
    }
  ...
  useEffect(() => {
    ...
    const tl = gsap.timeline({
      paused: true,
      onUpdate: () => {
        setDisplayProgress(tl.progress())
        redraw()
      },
    })

    tl.to(s, { linePct: 1, duration: DURATION * 0.4, ease: "power2.out" }, 0)
      .to(s, { labelOpacity: 1, duration: DURATION * 0.1 }, DURATION * 0.3)
      .to(s, { pointOpacity: 1, duration: DURATION * 0.1 }, DURATION * 0.45)
      .to(s, { angleOpacity: 1, duration: DURATION * 0.1 }, DURATION * 0.6)
      .to(s, { gradientOpacity: 1, duration: DURATION * 0.1 }, DURATION * 0.75)
